...
use crate::toml::{get_repo_links, remove_top_module};
...
fn remove_module(module_path: &str) -> Result<()> {
    ...
    print!(
        "Are you sure you want to remove the module {}? (y/n): ",
        module_name
    );
    io::stdout().flush()?;
    ...
    let repo_link = match repo_links.len() {
        0 => {
            return Err(anyhow!(
                "No repository links found for module: {}",
                module_name
            ))
        }
    ...
    print!(
        "To confirm removal, please re-type \"{}\" (without the quotes): ",
        module_name
    );
    io::stdout().flush()?;
